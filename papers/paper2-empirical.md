...
4. **Model scale does not fix GReaT's failure modes.** We replicate the GReaT protocol with Mistral-7B (7B parameters, 60× larger than GPT-2) on all three datasets. Mistral-7B shows marginal improvement on semantic-feature datasets but fails on anonymized features, underperforms CTGAN on Hillstrom, and exhibits higher generation failure rates than GPT-2 under extreme imbalance at small n. GReaT-fit variance (up to 12pp per-seed AUC drift) is documented for GPT-2; the underlying cause (non-deterministic GPU reductions) is model-agnostic.
...
#### Mistral-7B (7B parameters): Does Scale Fix GReaT's Failure Modes?

To test whether GPT-2's failures are a model-scale artifact rather than a fundamental limitation of the GReaT framework, we replicate the full fine-tuning protocol using Mistral-7B-v0.1 (7B parameters, 2024 — 60× larger than GPT-2) on all three GReaT datasets. Protocol is identical: same SEEDS, same SMALL_NS, same holdout, same GBC downstream classifier, same GReaT fine-tuning framework. Runs on H100 GPU with bf16 precision and 8-bit Adam optimizer.
...
**Key finding: model scale does not fix GReaT's failure modes.** Mistral-7B is marginally better than GPT-2 on semantic-feature datasets (Hillstrom, Telco) but the pattern is inconsistent and the gains remain small and unreliable. On anonymized features (German Credit), Mistral-7B also hurts across most n values — the LLM prior is inapplicable regardless of model size. Crucially, Mistral-7B's best Hillstrom gain (+3.55 pts at n=100) remains far below CTGAN (+5.75 pts). The generation failure at n=50 (Hillstrom) is more severe for Mistral-7B than GPT-2, suggesting that larger models may be more brittle under extreme imbalance at very small training sizes.

The GReaT-fit variance finding documented for GPT-2 likely extends to Mistral-7B (fp16 non-determinism on GPU is model-agnostic), though we did not formally measure it for the 7B model.
