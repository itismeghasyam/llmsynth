%python
# Phase 5 α-sweep — GReaT on German Credit at small-n shoulder.
# Pre-registered in papers/improvement_plan.md (Phase 5).
#
# Note: the data file is `credit_default.csv` for historical reasons; the
# underlying dataset is Statlog German Credit (OpenML id=31, ~30% positive).
#
...
#
# Output: {WORK_DIR}/great_alpha_sweep_german_results.csv
...
DATA_PATH = f"{WORK_DIR}/credit_default.csv"  # legacy filename; data is German Credit
OUT_PATH  = f"{WORK_DIR}/great_alpha_sweep_german_results.csv"
...
for n_train in SMALL_NS:
    ...
    for seed in SEEDS:
        ...
        # Random sampling (matches run_great_databricks.py)
        ...
        try:
            ...
            ckpt_dir = "/tmp/great_alpha_ckpt_german"
